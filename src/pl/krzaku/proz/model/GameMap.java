...
public class GameMap
{
	...
	private final int redMapColor = 116;
	// /Green component in RGB terrain color
	private final int greenMapColor = 96;
	// /Blue component in RGB terrain color
	private final int blueMapColor = 69;
	 ...
	 * 
	 * @param x
	 *            position along X axis of map
	 * @param howmuch
	 *            how high is terrain at point x
	 */
	public void setHeight(int x, int howmuch)
	{
		for (int i = 0; i < howmuch; i++)
		{
			mapImage.setRGBA(x, mapHeight - i, redMapColor, greenMapColor,
					blueMapColor, 255);
		}
		modified = true;
	}
	 ...
	 * 
	 * @param x
	 *            position of the circle's center along X axis
	 * @param y
	 *            position of the circle's center along X axis
	 * @param r
	...
	{
		int startX = x - r, endX = x + r, startY = y - r, endY = y + r;

		for (int i = startX; i <= endX; i++)
		{
			for (int j = startY; j <= endY; j++)
			{
				if ((i - x) * (i - x) + (j - y) * (j - y) <= r * r && i < mapWidth && i >= 0 && j < mapHeight && j > 0)	mapImage.setRGBA(i, j, 0, 0, 0, 0);
			}
		}
	...
	/**
	 * Returns base map width
	 * 
	 * @return base map width
	 */
	...
	/**
	 * Returns base map height
	 * 
	 * @return base map height
	 */
